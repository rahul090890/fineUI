			...
			return {
				...
				link : function($scope, $element, $attrs) {
					...
					$scope.texts = {
						...
						dynamicButtonTextSuffix : ' Project Selected'
					};
						...
						$scope.$watch('options', function(newValue) {
							if (angular.isDefined(newValue)) {
								$scope.orderedItems = $filter('orderBy')(
										newValue, $scope.settings.groupBy);
							}
						});
					}
					...
					$scope.selectAll = function() {
						...
						angular.forEach($scope.options, function(value) {
							$scope.setSelectedItem(
									value[$scope.settings.idProp], true);
						});
					};
					...
					$scope.setSelectedItem = function(id, dontRemove) {
						...
						if (!dontRemove && exists) {
							$scope.selectedModel.splice(_.findIndex(
					...
					$scope.isChecked = function(id) {
						...
						return _
								.findIndex($scope.selectedModel, getFindObj(id)) !== -1;
					};
